 ...
 */
export const annotateChildScopes = (nodes: BuildGraph['nodes']): void => {
 ...
 *
 * @param graph - The BuildGraph structure containing nodes, dependencies, and reverseDependencies.
 */
export const propagateScopes = (graph: BuildGraph): void => {
 ...
 * @param nodes - Map of JSON Pointer to NodeInfo.
 */
export const seedLocalScopes = (nodes: BuildGraph['nodes']): void => {
 ...
 */
export const buildGraph = (root: unknown): BuildGraph => {
  ...
  const walk = ({
    ...
    graph.nodes.set(pointer, { key, node, parentPointer });
