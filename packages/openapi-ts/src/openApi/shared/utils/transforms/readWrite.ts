...
const getComponentContext = (
  ...
  if (path.length === 4 && path[0] === '#' && path[1] === 'components') {
    const type = path[2];
 ...
 *
 * @param graph - The BuildGraph containing all nodes and their scopes
...
export const splitSchemas = ({
  ...
  const schemasNamespace = specToSchemasNamespace(spec);
  const schemasNamespaceSegments = schemasNamespace.split('/').length;
  ...
  const pointerToSchema = (pointer: string): string => {
    if (pointer.startsWith(schemasNamespace)) {
      const segments = pointer.split('/');
      if (segments.length === schemasNamespaceSegments) {
        return segments[schemasNamespaceSegments - 1] || '';
      }
    }
    return '';
  };
  ...
  for (const [pointer, nodeInfo] of graph.nodes) {
    ...
    const readPointer = `${schemasNamespace}${readName}`;
    ...
    const writePointer = `${schemasNamespace}${writeName}`;
  ...
  const walk = ({
    ...
    if (node instanceof Array) {
      ...
      if (isPathRootSchema(path)) {
        nextPointer = `${schemasNamespace}${path[3]}`;
